...
class RPMessage {
	...
	size_t length() const { return bodyLength()+2; }
...
{ 
	...
	CPError(unsigned wTIFlag=0, unsigned wTIValue=7)
		:CPMessage(wTIFlag, wTIValue),
		mCause(0x7f)
 	{ }

	CPError(unsigned wTIFlag, unsigned wTIValue, const CPCause& wCause)
		:CPMessage(wTIFlag, wTIValue),
		mCause(wCause)
 	{ }
...
class CPData : public CPMessage
{
	...
	CPData(unsigned wTIFlag, unsigned wTIValue, const RPMessage& RPM)
		:CPMessage(wTIFlag, wTIValue),
		mData(RPM)
 	{ }
