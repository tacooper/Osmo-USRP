...
{
	int maxAtten = gConfig.getNum("GSM.PowerManager.MaxAttenDB");
	int minAtten = gConfig.getNum("GSM.PowerManager.MinAttenDB");
	if (mAtten==minAtten) {
...
void PowerManager::reducePower()
{
	...
	int minAtten = gConfig.getNum("GSM.PowerManager.MinAttenDB");
	if (mAtten==maxAtten) {
		LOG(WARN) << "power already at minimum";
		return;
	}
...
{
	unsigned target = gConfig.getNum("GSM.PowerManager.TargetT3122");
...
void PowerManager::sampleT3122()
{
	...
	unsigned numSamples = gConfig.getNum("GSM.PowerManager.NumSamples");
...
PowerManager::PowerManager()
	: mAveragedT3122(gConfig.getNum("GSM.PowerManager.TargetT3122"))
	, mAtten(gConfig.getNum("GSM.PowerManager.MaxAttenDB"))
	, mNextSampleIndex(0)
{
	// We don't actually set any power here, since the radio may not exist yet.
	assert(gConfig.getNum("GSM.PowerManager.NumSamples")<100);
...
{
	while (1) {
		usleep(1000*gConfig.getNum("GSM.PowerManager.SamplePeriod"));
		sampleT3122();
		if (mLast.elapsed() < gConfig.getNum("GSM.PowerManager.Period")) continue;
		internalControlStep();
		mLast.now();
	}
}
